...
class DPGMultiPerceptronValueEstimator(object):
    ...
    def __init__(self, sess, state_dim, action_dim, h1_dim, h2_dim, learning_rate, tau, num_actor_vars):
        ...
        self._inputs, self._action, self._out = self._create_critic_network()

        self._network_params = tf.trainable_variables()[num_actor_vars:]

        # Target Network
        self._target_inputs, self._target_action, self._target_out = self._create_critic_network()
    ...
    def _create_critic_network(self):
        inputs = tflearn.input_data(shape=[None, self._s_dim])
        action = tflearn.input_data(shape=[None, self._a_dim])
